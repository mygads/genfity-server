...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!subcategory) {
      return withCORS(new NextResponse(JSON.stringify({ message: "Subcategory not found" }), {
        status: 404,
        headers: { "Content-Type": "application/json" },
      }));
    }
    ...
    // Create package with features in a transaction
...
export async function GET(request: NextRequest) {
  try {
    ...
    return withCORS(NextResponse.json(packages));
  } catch (error) {
